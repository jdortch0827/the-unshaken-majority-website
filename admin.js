(() => {
  ...
  async function requireAdmin() {
    ...
    $$('[data-admin-user]').forEach(el => el.textContent = `${result.profile.display_name} Â· ${result.profile.role}`);
  ...
  function renderCorrections(items=[]) {
    ...
    list.innerHTML=items.map(item=>`<article class="correction-review-card" data-correction-id="${escapeHtml(item.id)}">
      <header><div><h3>${escapeHtml(item.reference_number)} Â· ${escapeHtml(item.case_number)}</h3><div class="correction-review-meta">${escapeHtml(item.name)} Â· ${escapeHtml(item.email)} Â· ${escapeHtml(formatDate(item.created_at,true))}</div></div><span class="status-badge">${escapeHtml(item.status)}</span></header>
      <details><summary>Review request</summary><p><strong>Statement challenged:</strong><br>${escapeHtml(item.challenged_statement)}</p><p><strong>Explanation:</strong><br>${escapeHtml(item.explanation || '')}</p><p><strong>Requested correction:</strong><br>${escapeHtml(item.requested_correction)}</p>${item.source_url ? `<p><a href="${escapeHtml(item.source_url)}" target="_blank" rel="noopener noreferrer">Open supporting source â†—</a></p>` : ''}${(item.correction_attachments||[]).map(a=>a.admin_url?`<p><a href="${escapeHtml(a.admin_url)}" target="_blank" rel="noopener noreferrer">Open private attachment: ${escapeHtml(a.original_filename)} â†—</a></p>`:'').join('')}</details>
  ...
  async function initDashboard() {
    ...
    setStatus(status,'Loading investigationsâ€¦');
    try {
  ...
  async function saveEditor(autosave=false){
    ...
    state.saving=true;const saveState=$('#editor-save-state');if(saveState){saveState.textContent=autosave?'Autosavingâ€¦':'Savingâ€¦';saveState.className='save-state is-saving';}
  ...
  async function initEditor(){
    ...
    if(isNew){$('#admin-editor-loading').hidden=true;$('#new-investigation-panel').hidden=false;const dash=await jsonFetch('/api/admin-investigations');state.categories=dash.categories||[];fillSelect($('#new-category'),state.categories.map(c=>({value:c.id,label:c.name})),'','Uncategorized');$('#new-investigation-form').addEventListener('submit',async e=>{e.preventDefault();const status=$('#new-investigation-status');setStatus(status,'Creating case fileâ€¦');try{const result=await jsonFetch('/api/admin-investigations',{method:'POST',body:JSON.stringify({action:'create',title:$('#new-title').value,subject:$('#new-subject').value,categoryId:$('#new-category').value||null,shortSummary:$('#new-summary').value})});location.replace(`/admin/investigations/${result.investigation.id}/edit`);}catch(error){setStatus(status,error.message,'error');}});return;}
